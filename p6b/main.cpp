...
private:
	...
	std::shared_ptr<CheckBoxWidget> enablePhong;
...
void MyRender::setup() {
	...
	gouraud.loadFiles("../p6b/gouraud");
	gouraud.compile();

	uniformPhong = gouraud.getUniformLocation("phong");
...
void MyRender::render() {
	...
	glUniform1i(uniformPhong, enablePhong->get());
...
void MyRender::buildGUI() {
	...
	// Creamos un checkbox para activar o desactivar el phong
	enablePhong = std::make_shared<CheckBoxWidget>("Activar phong", false);
	// Añadimos el checkbox en el panel
	panel->addWidget(enablePhong);
}
